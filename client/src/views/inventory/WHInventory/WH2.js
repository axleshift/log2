...
const Inventory = () => {
  ...
  // Get the warehouseId from the URL
  const { warehouseId } = useParams()

  useEffect(() => {
    const fetchInventoryData = async () => {
      try {
        // Fetch inventory data for the specific warehouseId
        const response = await axios.get(`http://localhost:5058/api/v1/inventory/${warehouseId}`)
    ...
    if (warehouseId) {
      fetchInventoryData() // Only fetch data if warehouseId is available
    }
  }, [warehouseId])
  ...
  const handleSelectAllChange = (e) => {
    ...
    if (isChecked) {
      setSelectedRows(new Set(inventoryData.map((_, index) => index)))
    } else {
